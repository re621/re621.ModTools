...
export default class ReportedContentAge extends Component {
	...
	public constructor() {
		super({
			...
			waitForDOM: `${ReportedContentAge.tableSelector} tr td:nth-child(2) a:first-child[href^="/comments/"]`,
		});
	}

	protected async create(): Promise<void> {
		const reportedRow = ReportedContentAge.findReportedCommentRow();
		if (!reportedRow) return;

		const link = reportedRow.children[1].querySelector<HTMLAnchorElement>('a[href^="/comments/"]');
		if (!link) return;

		const ageRow = ReportedContentAge.makeAgeRow(reportedRow);
