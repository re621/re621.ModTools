...
export default class TimeKeeper extends Component {
  ...
  protected create(): Promise<void> {
    ...
    this.on("settings.enabled", (ev, d) => {
      if (!d) Timestamp.teardownAll();
      else Timestamp.tryInitAll();
    });
    Timestamp.tryInitAll();
  ...
  private initSettingsMenu() {
    this.settingsMenuDialogParameters = {
      ...
      optionsOrTitle: "Time Keeper Settings",
      then: (e: FormData) => {
...
class Timestamp {
  ...
  public static initAll () {
    Array.from(document.querySelectorAll("time")).forEach((e) => {
      if (!this.instances.some(e1 => e1.element === e)) new Timestamp(e);
    });
  }
  private static readonly instances: Timestamp[] = [];
  public static teardownAll() {
    this.instances.forEach(e => e.teardown());
  ...
  public readonly toggleIcon: HTMLSpanElement | undefined;
  public teardown() {
    (this.toggleIcon ?? this.element).removeEventListener("click", this.toggleDisplayedStyle);
    this.toggleIcon?.remove();
    if (this.isToggleActive) this.toggleDisplayedStyle();
    this.stopUpdating();
  }
  ...
  constructor (
    ...
    for (const range of Timestamp.ranges) {
      ...
      Timestamp.instances.push(this);
      return;
    }
    console.warn("Should have matched a pattern (text: %s, element: %o)", currentText, element);
    throw new Error("Failed to match a pattern");
  }

  public static tryInit(element: HTMLTimeElement) {
    try {
  ...
  private scheduleNextUpdate (doUpdateIfDue = true) {
    ...
    if (updateTime !== undefined) {
      if (updateTime <= 0) {
        ...
        this.timerID = window.setTimeout(() => this.update(), updateTime);
        return true;
      }
    } else {
      this.stopUpdating();
  ...
  /** 1440   */ private static readonly MINUTES_IN_DAY = 24 * 60;
  /** 525600 */ private static readonly MINUTES_IN_YEAR = this.MINUTES_IN_DAY * 365;
  /** 131400 */ private static readonly MINUTES_IN_QUARTER_YEAR = this.MINUTES_IN_YEAR / 4;
  /** 394200 */ private static readonly MINUTES_IN_THREE_QUARTERS_YEAR = this.MINUTES_IN_QUARTER_YEAR * 3;
  ...
  private static findSecondsDeltaFormat (deltaSeconds: number): FormatInstanceData {
    switch (true) {
  ...
  private static findYearDeltaFormat (fromTime: Date, toTime: Date, deltaMinutes = Math.round((toTime.valueOf() - fromTime.valueOf()) / (1000 * 60))): Required<FormatInstanceData> {
